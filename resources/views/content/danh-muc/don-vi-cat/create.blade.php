...
<div class="card">
    <div class="card-header d-flex flex-column flex-md-row gap-3 justify-content-between align-items-md-center">
        <h5 class="mb-0">Thêm đơn vị cắt</h5>
        <a href="{{ route('don-vi-cat.index') }}" class="btn btn-outline-secondary">
            <i class="icon-base bx bx-arrow-back me-1"></i> Quay lại
        </a>
    </div>
    <div class="card-body">
        <form action="{{ route('don-vi-cat.store') }}" method="POST">
            @csrf

            <div class="row g-4">
                <div class="col-md-4">
                    <label class="form-label" for="ma_don_vi">Mã đơn vị <span class="text-danger">*</span></label>
                    <input type="text" class="form-control @error('ma_don_vi') is-invalid @enderror" id="ma_don_vi" name="ma_don_vi" value="{{ old('ma_don_vi') }}" maxlength="50" required>
                    @error('ma_don_vi')
                ...
                <div class="col-md-8">
                    ...
                    <input type="text" class="form-control @error('ten_don_vi') is-invalid @enderror" id="ten_don_vi" name="ten_don_vi" value="{{ old('ten_don_vi') }}" required>
                    @error('ten_don_vi')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
